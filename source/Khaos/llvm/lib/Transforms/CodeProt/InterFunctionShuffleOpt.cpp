...
bool InterFunctionShuffleOpt::tryInlineFunction(Function &F)
{
    ...
    if (F.isDeclaration() || F.hasFnAttribute(Attribute::NoInline) || F.isCreatedByCodeProt() || !isInlineViable(F))
    {
        return Changed;
    }
...
bool InterFunctionShuffleOpt::runOnModule(Module &M) {
    ...
    // uint SplitCount = 0;
    // if (!M.getName().startswith("ga") && !M.getName().startswith("gb") &&
    //     !M.getName().startswith("gc") && !M.getName().startswith("gd") &&
    //     !M.getName().startswith("ge") && !M.getName().startswith("gf") &&
    //     !M.getName().startswith("gg") && !M.getName().startswith("gh") &&
    //     !M.getName().startswith("gimpli") && !M.getName().startswith("g") &&
    //     !M.getName().startswith("k") && !M.getName().startswith("l") &&
    //     !M.getName().startswith("y") && !M.getName().startswith("z"))
    //     return false;
    ...
    for (Function &F : M) 
    {
        ...
        //functions created by CodeProt, std functions
		if (F.isIntrinsic() || F.isDeclaration() || F.empty() || F.hasOptNone() ||
            F.isCreatedByCodeProt() || F.getName().find("std", 0) != StringRef::npos ||
            F.getName().find("INS_6VectorIdEEE5solveIN") != StringRef::npos) 
            continue;
        // if (F.getName().find("Perl_regexec_flags") != StringRef::npos ||
        //     F.getName().find("S_find_byclass") != StringRef::npos ||
        //     F.getName().find("S_regmatch") != StringRef::npos ||
        //     F.getName().find("S_regtry") != StringRef::npos ||
        //     F.getName().find("Perl_leave_scope") != StringRef::npos ||
        //     F.getName().find("Perl_save_alloc") != StringRef::npos ||
        //     F.getName().find("Perl_runops_standard") != StringRef::npos ||
        //     F.getName().find("S_regrepeat") != StringRef::npos ||
        //     F.getName().find("Perl_pp_match") != StringRef::npos ||
        //     F.getName().find("Perl_sv_free") != StringRef::npos ||
        //     F.getName().find("S_hv_fetch_common") != StringRef::npos ||
        //     F.getName().find("_ZNK10xalanc_1_818ReusableArenaBlockINS_13XStringCachedEE10ownsObjectEPKS1") != StringRef::npos || //483
        //     F.getName().find("_ZN11xercesc_2_523NCNameDatatypeValidator7compareEPKtS2_PNS_13MemoryManagerE") != StringRef::npos ||  //483
        //     F.getName().find("_ZNK10xalanc_1_811XalanBitmap5isSetEm") != StringRef::npos ||                                     //483
        //     F.getName().find("_ZNK10xalanc_1_818ReusableArenaBlockINS_13XStringCachedEE14blockAvailableEv") != StringRef::npos  //483
        //     ) continue;
        ...
        // if (SplitCount > 26)
        //     continue;
        // if (!F.getName().equals("_ZN6Cmdenv8simulateEv"))
        //     continue;
